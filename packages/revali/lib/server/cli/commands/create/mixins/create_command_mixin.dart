...
import 'package:revali/server/cli/models/cli_config.dart';
...
mixin CreateCommandMixin {
  ...
  CliConfig get config {
    ...
    if (yaml['server'] case final Map<dynamic, dynamic> config) {
      logger.detail('Found revali.yaml');
      return CliConfig.fromJson(config);
    }

    logger.detail('No server config found in revali.yaml');
