...
abstract class Message {
  ...
  }) : this._withBody(Payload(body, encoding), headers);

  Message._withBody(Payload body, Map<String, Object>? headers)
      : this._withHeadersAll(
          body,
          _adjustHeaders(expandToHeadersAll(headers), body),
        );

  Message._withHeadersAll(Payload body, Map<String, List<String>> headers)
      : _body = body,
        _headers = headers;
...
bool _sameEncoding(Map<String, List<String>?>? headers, Payload body) {
...
extension _HeadersX on Map<String, List<String>> {
  Map<String, String> get singleValues => CaseInsensitiveMap.fromEntries(
        entries.map((entry) {
          return MapEntry(entry.key, entry.value.join(','));
        }),
      );
}
