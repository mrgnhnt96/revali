...
void main() {
  group('future', () {
    ...
    test('user-type', () async {
      final response = await client.future.userType();

      expect(response, UserType.admin);
      verifyGetRequest('/api/future/user-type');
    });

    test('serialized-user-type', () async {
      final response = await client.future.serializedUserType();

      expect(response, SerializedUserType.admin);
      verifyGetRequest('/api/future/serialized-user-type');
    });
  });
}
