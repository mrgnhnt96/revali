...
void main() {
  group('header params', () {
    ...
    test('required', () async {
      final response = await client.headerParams.required(shopId: '123');

      expect(response, '123');
      verifyRequest('/api/header/required', method: 'GET');
      expect(request?.headers['X-Shop-Id'], '123');
    });

    test('optional', () async {
      final response = await client.headerParams.optional(shopId: '123');
    ...
    test('all', () async {
      final response = await client.headerParams.all(shopIds: ['123', '456']);

      expect(response, '123,456');
      verifyRequest('/api/header/all', method: 'GET');
      expect(request?.headers['X-Shop-Id'], '123,456');
    });
