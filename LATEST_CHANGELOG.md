...
- Add `RequestObserver` and `RequestSummary`, giving observers what `Observer` could not: a completed request's method, path, matched **route** path, status, duration and error. Metrics should be labelled with `routePath` (`/api/users/:id`) rather than `path` (`/api/users/42`), which is the difference between one time series and one per id. Implemented as a second interface alongside `Observer` rather than a change to it, so no existing observer breaks.
